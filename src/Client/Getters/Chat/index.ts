import { AskResourcePayload, ChatData, ContactID, GroupChatData, PrivateChatData } from 'kozz-types';
...
const getGroupData = async (chatData: GroupChat, whatsappBoundary: Client): Promise<GroupChatData> => {
    const allAdminsContacts = await Promise.all(
        chatData.participants
            .filter(p => p.isAdmin || p.isSuperAdmin)
            .map(admin => admin.id).map(id =>
                whatsappBoundary
...
const getPrivateChatData = async (chatId: string, whatsappBoundary: Client): Promise<PrivateChatData> => {
